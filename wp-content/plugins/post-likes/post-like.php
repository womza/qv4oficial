...
* Author http://jonmasterson.com/
* License: Copyright (C) 2014 Jon Masterson
...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', get_template_directory_uri().'/js/min/post-like.min.js', array('jquery'), '1.0', 1 );
...
function getPostLikeLink( $post_id ) {
	...
	if ( AlreadyLiked( $post_id ) ) {
		...
		$heart = '<i class="icon-like fa fa-heart"></i>';
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( 'Me gusta' );
		$heart = '<i class="icon-unlike fa fa-heart-o"></i>';
	}
	$output = '<a href="#" class="jm-post-like'.$class.' ia" data-post_id="'.$post_id.'" title="'.$title.'">'.$count.' '.$heart.'</a>';
	return $output;
}


/* don't really need below */
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'sk8er' ); ?></label></th>
			<td>
            <?php
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . __( 'You Like:', 'sk8er' ) . "</h3>\n";
			$like_list .= "<ul>\n";
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . _e( 'Most Popular Posts', 'sk8er' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_key' => '_post_like_count',
